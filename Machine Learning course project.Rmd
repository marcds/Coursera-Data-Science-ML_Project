...
rm(nzero, training, validation, trainset, val_no_na, testing)
```
...
```{r}
# To avoid overfitting and to reduce out of sample errors, we checked variables correlation and included the PCA pre-processing.

corr <- findCorrelation(cor(train_set[, -53]), cutoff=0.8)
names(train_set)[corr]

tr_control <-  trainControl(method = "cv", number = 4, preProcOptions="pca", classProbs=TRUE, savePred = TRUE, allowParallel=TRUE)
...
library(kernlab)
svmR <- train(classe ~ ., data = train_set, method = "svmRadial", tuneLength = 15, trControl = tr_control)
...
```
The accuracy of the model is **99.37%** with a confidence interval between **99.13-99.56%.**
